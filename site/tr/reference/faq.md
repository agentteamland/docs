...
Evet. Her kurulum kendi sembolik linklerini `.claude/` altına ekler. İki takım aynı adla bir agent sunuyorsa **ikinci kurulan** takımın versiyonu kazanır (ilk sembolik linkin üzerine yazılır). Her sembolik linki hangi takımın sahiplendiğini `atl list` ile görebilirsin.
...
Evet. Takım reposunu `~/.claude/repos/agentteamland/<takım-adı>/` altına klonla ve sembolik linkleri `.claude/` altında sen oluştur. CLI; bunu artı miras/excludes çözümünü otomatikleştirmek için var; çıktısında büyülü bir şey yok.
...
Hayır. `atl` yerel bir araç: GitHub'dan klonlar, registry'yi çeker, sembolik link yazar. Phone-home yok.
